...
def assign_shifts(employees):
    ...
    for e in employees:
        shifts=[e[2][0]+i*3 for i in range(1,construction_config_antibiotics.days)]    #same shift each working day
        e[2].extend(shifts)
...
def employeeGenerator():
    ...
    profession_levels = np.random.choice(construction_config_antibiotics.professionLevels, 
                                         construction_config_antibiotics.E_num, 
                                         p=construction_config_antibiotics.professionLevelsProb)
...
def employeeGeneratorOnlyDay():
    ...
    if construction_config_antibiotics.E_num > 5:
        first_levels = np.array([1, 2, 2, 3, 3])
        remaining_levels = np.random.choice(construction_config_antibiotics.professionLevels, 
                                         construction_config_antibiotics.E_num-5, 
                                         p=construction_config_antibiotics.professionLevelsProb)
        profession_levels = np.concatenate([first_levels, remaining_levels])
    elif construction_config_antibiotics.E_num == 5:
        profession_levels = np.array([1, 2, 2, 3, 3])
    elif construction_config_antibiotics.E_num == 4:
        profession_levels = np.array([1, 2, 3, 3])
    elif construction_config_antibiotics.E_num == 3:
        profession_levels = np.array([2, 3, 3])
    elif construction_config_antibiotics.E_num == 2:
    ...
    # Forbered klinikker
    clinic = np.empty(construction_config_antibiotics.E_num, dtype=int)
    ...
    for i, level in enumerate(profession_levels):
        if level == 1:
            ...
            if np.random.rand() <= construction_config_antibiotics.E_generalists:
                clinic[i] = 0
            else:
                clinic[i] = np.random.choice(range(2, len(construction_config_antibiotics.clinicDistribution) + 2), p=construction_config_antibiotics.clinicDistribution)
